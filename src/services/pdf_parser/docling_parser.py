# This module provides PDF parsing for scientific papers using Docling as the
# core parsing engine and pypdfium2 for lightweight validation.

# Before parsing, every PDF goes through four validation checks — empty file,
# file size limit, valid PDF header (magic bytes), and page count limit. Any
# violation raises a PDFValidationError; size and page limit violations return
# None instead of crashing so oversized papers are silently skipped.

# Parsing is done by Docling which renders each page for layout analysis,
# detects headings and text blocks, and reconstructs the document structure.
# The output is a PdfContent object containing a list of PaperSection objects
# where each section groups all text under its heading — extracted using a
# sliding window pattern that accumulates text until the next heading is found.
